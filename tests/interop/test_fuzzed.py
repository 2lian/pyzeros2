...
async def py0_to_ros_pubsub(
    rclpy_init, msg_type: Type[idl.IdlStruct]
) -> FuzzPubSub:
    ros_topic = afor.TopicInfo(
    ...
    with auto_context(node="fuzz_py0_to_ros"):
...
async def ros_to_py0_pubsub(
    ...
    with auto_context(node="fuzz_ros_to_py0"):
...
async def test_ros_to_py0_fuzzed(ros_to_py0_pubsub: FuzzPubSub):
    ...
    for py0_msg in fuzzed_messages(msg_type):
        async with TaskGroup() as tg:
            publish_task = tg.create_task(periodic(py0_msg))
            recv_message = await asyncio.wait_for(sub.wait_for_new(), RECV_TIMEOUT_S)
